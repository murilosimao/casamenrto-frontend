        ...
        await request(HTTP_GET, `/api/comment?per=${pagination.getPer()}&next=${pagination.getNext()}`)
            ...
            .then((res) => {
                ...
                if (res.data.length === 0) {
                    comments.innerHTML = `<div class="h6 text-center fw-bold p-4 my-3 bg-theme-${theme.isDarkMode('dark', 'light')} rounded-4 shadow">Vamos compartilhar esse convite para que haja muitos comentários</div>`;
                    return;
                }
